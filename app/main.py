...
async def _serve_opencode_go_responses(request: Request, s, body: dict) -> Response:
    """Serve Responses through the strategy selected for this model protocol."""
    gateway_model = str(body.get("model") or "")
    upstream_model = strip_opencode_go_model(gateway_model)
    protocol = (
        ResponsesUpstreamProtocol.NATIVE
        if upstream_model in OPENCODE_GO_RESPONSES_MODELS
        else ResponsesUpstreamProtocol.ANTHROPIC_MESSAGES
        if upstream_model in OPENCODE_GO_MESSAGES_MODELS
        else ResponsesUpstreamProtocol.CHAT_COMPLETIONS
    )
    try:
        adapter = ResponsesAdapterFactory.create(protocol)
        prepared = adapter.prepare_request(body, upstream_model)
    except UnsupportedResponsesFeature as exc:
        return _error(400, str(exc), "unsupported_feature", "invalid_request_error")

    async def open_upstream():
        if protocol is ResponsesUpstreamProtocol.NATIVE:
            return await s.opencode_go_proxy.open_responses(prepared.body)
        if protocol is ResponsesUpstreamProtocol.ANTHROPIC_MESSAGES:
            return await s.opencode_go_proxy.open_messages(prepared.body)
        return await s.opencode_go_proxy.open_chat(prepared.body)

    translate = adapter.requires_response_translation(prepared)
    ...
    if body.get("stream"):
        ...
        async def gen():
            try:
                async for event in adapter.translate_stream(
                    iter_sse(upstream.aiter_bytes()), gateway_model, prepared
                ):
                    yield event
            finally:
                await upstream.aclose()
    ...
    transform = None
    if translate:
        transform = lambda raw: json.dumps(adapter.translate_response(
            json.loads(raw), gateway_model, prepared
        )).encode()
...
async def _serve_cursor_responses(s, body: dict) -> Response:
    ...
    try:
        adapter = ResponsesAdapterFactory.create(
            ResponsesUpstreamProtocol.TEXT_GENERATION
        )
        prepared = adapter.prepare_request(body, model.removeprefix("cursor/"))
    except UnsupportedResponsesFeature as exc:
        return _error(400, str(exc), "unsupported_feature", "invalid_request_error")
    prompt = cursor_prompt_from_responses(prepared.body.get("input"))
    ...
    await s.router.record_success(account)
    return JSONResponse(
        adapter.translate_response(payload, model, prepared),
        headers={"X-Gateway-Account": account.id},
    )
